...
import com.qagwaai.starmalaccamax.client.admin.util.HTML5BulkClosestLoadCommand;
...
import com.qagwaai.starmalaccamax.client.data.ClosestDS;
 ...
 * 
 */
public final class ClosestAdminPresenterImpl extends AbstractPresenter<ClosestAdminView, Closest> implements
    ClosestAdminPresenter {
     ...
     */
    public ClosestAdminPresenterImpl(final EventBus eventBus, final ClosestAdminViewImpl view, final Closest model) {
        super(eventBus, view, model);
        // make sure to do the association before calling layout
        view.setPresenter(this);
        try {
            dataSource = DataSourceFactory.get(ClosestDS.DS_TYPE, eventBus);
        } catch (InvalidParameterException e) {
            view.alert(ClosestAdminPresenterImpl.constants.dataSourceError());
            return;
        }
        view.layout();

        HTML5BulkLoadCommand loadCommand = new HTML5BulkClosestLoadCommand((AdminFileUpload) getView());
